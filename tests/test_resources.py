...
def write_skill_with_resources(directory: Path, name: str = "TestSkill") -> Path:
...
def test_resource_mime_types(tmp_path: Path) -> None:
    ...
    # Check MIME types
    assert resources_by_name["testskill/SKILL.md"]["mime_type"] == "text/markdown"
    assert resources_by_name["testskill/script.py"]["mime_type"] == "text/x-python"
    assert resources_by_name["testskill/README.md"]["mime_type"] == "text/markdown"
    # Binary files may not have a detected MIME type
    assert resources_by_name["testskill/data.bin"]["mime_type"] in [None, "application/octet-stream"]
